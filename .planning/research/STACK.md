...
**Domain:** Tauri 2 desktop app — AI-powered romaji-to-Japanese/English text conversion
**Researched:** 2026-04-05
**Confidence:** HIGH (Rust crates verified via docs.rs; Tauri via GitHub releases; JS ecosystem via npm/official docs)
...
**vite.config.ts — Tailwind v4 setup:**
```typescript
...
```css
@import "tailwindcss";
/* Dark green theme tokens go here with @theme */
```

**src-tauri/Cargo.toml — key crates:**
```toml
[dependencies]
...
**For AI streaming (token-by-token output):**
...
**For SQLite access from async Tauri commands:**
...
**For theming (dark green default + light mode):**
...
**For offline AI (Ollama / LM Studio):**
...
**Confidence: MEDIUM (based on GitHub ToS analysis)**

The GitHub Copilot API (`api.githubcopilot.com/chat/completions`) is intended only for officially supported clients (VS Code, JetBrains, CLI). The Device Flow OAuth approach used by third-party projects (requesting `copilot` scope on GitHub OAuth) is:

1. Not officially documented as a public API
2. Potentially a ToS violation for production/commercial use
3. Subject to silent breakage when GitHub changes internal endpoints

**Recommendation:** Implement `CopilotAdapter` as an explicitly opt-in, experimental feature behind a UI warning ("This uses an unofficial API. Use at your own risk."). Do not include it in the initial MVP. For team distribution, the Anthropic and OpenAI-compatible adapters are sufficient and legally unambiguous.
...
---
*Stack research for: Tauri 2 + React + TypeScript desktop app with AI text conversion*
*Researched: 2026-04-05*
